...
struct System {
  static void profile(const std::string &name, std::function<void()> body) {
    ...
    if (pid == 0) {
      ...
      dup2(fd, 1);
      dup2(fd, 2);
      exit(execl("/usr/bin/perf", "perf", "record", "-o", filename.c_str(),
      ...
      sprintf(
          buf,
          "perf stat -e "
          "cache-misses,cache-references,L1-dcache-load-misses,LLC-loads,LLC-"
          "load-misses,LLC-stores,LLC-store-misses,r412e -p %d > %s 2>&1",
          ppid, filename.c_str());
