...
public:
  ...
  CacheHeader(const std::vector<uint8_t> &byte_array, int depth,
              int partial_len)
      : depth(depth) {
...
using CacheMap =
    tbb::concurrent_unordered_map<uint8_t, CacheNodeValue, no_hash>;
...
public:
  ...
  CacheNode() { header = new CacheHeader(); }

  // insert leaf node
  CacheNode(const std::vector<uint8_t> &byte_array, int start,
  ...
  CacheNode(const std::vector<uint8_t> &byte_array, int start, int partial_len,
            uint8_t partial_1, CacheNode *next_node, uint8_t partial_2,
            CacheEntry *new_entry, CacheNode *&nested_node) {
    ...
    if (partial_1 == partial_2) { // split for insert new_entry at old header
      ...
      if (start + partial_len >=
          (int)byte_array.size() - 1) { // insert entry directly
        nested_node = nullptr;
        records[partial_2] = CacheNodeValue(new_entry, nullptr);
      } else { // insert leaf node
        nested_node =
            new CacheNode(byte_array, start + partial_len + 1, new_entry);
  ...
  uint64_t content_size() const {
    return ((CacheHeader *)header)->content_size() +
           (sizeof(uint8_t) + sizeof(CacheEntry *) + sizeof(CacheNode *)) *
               records.size();
  }
...
public:
  ...
  void free(int _size) { free_size.fetch_add(_size); }

  int64_t remain_size() const { return free_size.load(); }
...
struct SearchRet {
  ...
  // uint64_t counter;
  SearchRet() {}
  SearchRet(volatile CacheEntry **entry_ptr_ptr, CacheEntry *entry_ptr,
            int next_idx)
