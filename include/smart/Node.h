...
#define TREE_ENABLE_FINE_GRAIN_NODE 1
#define NO_MASK 1
...
thread_local static CRCProcessor crc_processor;
...
public:
  ...
  Leaf(const Key &key, const Value &value, const GlobalAddress &rev_ptr)
      : rev_ptr(rev_ptr), f_padding(0), valid(1), key(key), value(value),
        lock_byte(0) {
    set_consistent();
  }
  ...
  bool is_consistent() const {
    crc_processor.reset();
    crc_processor.process_bytes(
        (char *)&key, sizeof(Key) + sizeof(uint8_t) * define::simulatedValLen);
    return crc_processor.checksum() == checksum;
  }

  uint64_t recompute_checksum() const {
    crc_processor.reset();
    crc_processor.process_bytes(
        (char *)&key, sizeof(Key) + sizeof(uint8_t) * define::simulatedValLen);
...
enum NodeType : uint8_t { NODE_DELETED, NODE_4, NODE_16, NODE_48, NODE_256 };
#endif
...
public:
  Header() : depth(0), node_type(0), partial_len(0) {
    memset(partial, 0, sizeof(uint8_t) * define::hPartialLenMax);
  }
  Header(int depth) : depth(depth), node_type(0), partial_len(0) {
    memset(partial, 0, sizeof(uint8_t) * define::hPartialLenMax);
  }
  Header(NodeType node_type) : depth(0), node_type(node_type), partial_len(0) {
    memset(partial, 0, sizeof(uint8_t) * define::hPartialLenMax);
  }
  Header(const Key &k, int partial_len, int depth, NodeType node_type)
      : depth(depth), node_type(node_type), partial_len(partial_len) {
    assert((uint32_t)partial_len <= define::hPartialLenMax);
    for (int i = 0; i < partial_len; ++i)
      partial[i] = get_partial(k, depth + i);
  }
  ...
  bool is_match(const Key &k) {
    for (int i = 0; i < partial_len; ++i) {
      if (get_partial(k, depth + i) != partial[i])
        return false;
    }
    return true;
  }
  ...
  static Header split_header(const Header &old_hdr, int diff_idx) {
    auto new_hdr = Header();
    for (int i = diff_idx + 1; i < old_hdr.partial_len; ++i)
      new_hdr.partial[i - diff_idx - 1] = old_hdr.partial[i];
  ...
  static const uint64_t node_type_mask =
      (((1UL << define::nodeTypeNumBit) - 1) << 8);
} __attribute__((packed));
...
public:
  ...
  InternalEntry(uint8_t partial, uint8_t kv_len, const GlobalAddress &addr)
      : _partial(partial), kv_len(kv_len), _is_leaf(1),
        _packed_addr{addr.nodeID, addr.offset >> ALLOC_ALLIGN_BIT} {}
  InternalEntry(uint8_t partial, NodeType node_type, const GlobalAddress &addr)
      : partial(partial), empty(0), node_type(static_cast<uint8_t>(node_type)),
        is_leaf(0), packed_addr{addr.nodeID, addr.offset >> ALLOC_ALLIGN_BIT} {}
  InternalEntry(uint8_t partial, const InternalEntry &e)
      : _partial(partial), kv_len(e.kv_len), _is_leaf(e._is_leaf),
        _packed_addr(e._packed_addr) {}
  InternalEntry(NodeType node_type, const InternalEntry &e)
      : partial(e.partial), empty(0),
        node_type(static_cast<uint8_t>(node_type)), is_leaf(e.is_leaf),
        packed_addr(e.packed_addr) {}
  ...
  GlobalAddress addr() const {
    return GlobalAddress{packed_addr.mn_id,
                         packed_addr.offset << ALLOC_ALLIGN_BIT};
  }
} __attribute__((packed));

inline bool operator==(const InternalEntry &lhs, const InternalEntry &rhs) {
...
public:
  ...
  InternalPage(const Key &k, int partial_len, int depth, NodeType node_type,
               const GlobalAddress &rev_ptr)
      : rev_ptr(rev_ptr), hdr(k, partial_len, depth, node_type) {
    std::fill(records, records + 256, InternalEntry::Null());
  }

  bool is_valid(const GlobalAddress &p_ptr, int depth, bool from_cache) const {
    return hdr.type() != NODE_DELETED && hdr.depth <= depth &&
           (!from_cache || p_ptr == rev_ptr);
  }
} __attribute__((packed));
...
/*
  Range Query
*/
enum State : uint8_t { INSIDE, BORDER, OUTSIDE };
...
struct RangeCache {
  ...
  RangeCache() {}
  RangeCache(const Key &from, const Key &to, const GlobalAddress &e_ptr,
             const InternalEntry &e, int depth,
             volatile CacheEntry **entry_ptr_ptr, CacheEntry *entry_ptr)
      : from(from), to(to), e_ptr(e_ptr), e(e), depth(depth),
        entry_ptr_ptr(entry_ptr_ptr), entry_ptr(entry_ptr) {}
};
...
struct ScanContext {
  ...
  State r_state;
  ScanContext() {}
  ScanContext(const InternalEntry &e, const GlobalAddress &e_ptr, int depth,
              bool from_cache, volatile CacheEntry **entry_ptr_ptr,
              CacheEntry *entry_ptr, const Key &from, const Key &to,
              State l_state, State r_state)
      : e(e), e_ptr(e_ptr), depth(depth), from_cache(from_cache),
        entry_ptr_ptr(entry_ptr_ptr), entry_ptr(entry_ptr), from(from), to(to),
        l_state(l_state), r_state(r_state) {}
};
