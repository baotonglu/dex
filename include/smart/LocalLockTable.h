...
struct LocalLockNode {
  ...
  LocalLockNode()
      : read_current(0), read_ticket(0), read_handover(0), write_current(0),
        write_ticket(0), write_handover(0), window_start(0), read_window(0),
        write_window(0), unique_read_key(0), unique_write_key(0),
        unique_addr(0), handover_cnt(0) {}
};
...
public:
  ...
  // read-delegation
  std::pair<bool, bool>
  acquire_local_read_lock(const Key &k, CoroQueue *waiting_queue = nullptr,
                          CoroContext *cxt = nullptr, int coro_id = 0);
  void release_local_read_lock(const Key &k, std::pair<bool, bool> acquire_ret,
                               bool &res, Value &ret_value);

  // write-combining
  std::pair<bool, bool>
  acquire_local_write_lock(const Key &k, const Value &v,
  ...
  /* ---- baseline ---- */
  // lock-handover
  bool acquire_local_lock(const GlobalAddress &addr,
                          CoroQueue *waiting_queue = nullptr,
                          CoroContext *cxt = nullptr, int coro_id = 0);
  ...
  // write_testing
  bool acquire_local_write_lock(const GlobalAddress &addr, const Value &v,
                                CoroQueue *waiting_queue = nullptr,
                                CoroContext *cxt = nullptr, int coro_id = 0);
  using RemoteWriteBackFunc = std::function<void(const Value &)>;
  void release_local_write_lock(const GlobalAddress &addr,
                                RemoteFunc unlock_func, const Value &v,
                                RemoteWriteBackFunc write_func);

  // read-testing
  bool acquire_local_read_lock(const GlobalAddress &addr,
                               CoroQueue *waiting_queue = nullptr,
                               CoroContext *cxt = nullptr, int coro_id = 0);
  void release_local_read_lock(const GlobalAddress &addr, bool &res,
                               Value &ret_value);
...
LocalLockTable::acquire_local_read_lock(const Key &k, CoroQueue *waiting_queue,
...
LocalLockTable::release_local_read_lock(const Key &k,
  ...
  if (!node.read_handover &&
    ...
    node.write_window =
        ((1UL << 8) + node.write_ticket.load(std::memory_order_relaxed) -
         w_current) %
        (1UL << 8);
    node.w_lock.unlock();
  }
...
LocalLockTable::release_local_write_lock(const Key &k,
  ...
  if (!node.write_handover &&
    ...
    node.read_window =
        ((1UL << 8) + node.read_ticket.load(std::memory_order_relaxed) -
         r_current) %
        (1UL << 8);
    node.r_lock.unlock();
...
// lock-handover
inline bool LocalLockTable::acquire_local_lock(const GlobalAddress &addr,
                                               CoroQueue *waiting_queue,
                                               CoroContext *cxt, int coro_id) {
...
// lock-handover
inline void LocalLockTable::release_local_lock(const GlobalAddress &addr,
...
// write-testing
inline bool LocalLockTable::acquire_local_write_lock(const GlobalAddress &addr,
                                                     const Value &v,
                                                     CoroQueue *waiting_queue,
                                                     CoroContext *cxt,
                                                     int coro_id) {
...
// write-testing
inline void
LocalLockTable::release_local_write_lock(const GlobalAddress &addr,
                                         RemoteFunc unlock_func, const Value &v,
                                         RemoteWriteBackFunc write_func) {
  auto &node = local_locks[hasher.get_hashed_lock_index(addr)];
...
// read-testing
inline bool LocalLockTable::acquire_local_read_lock(const GlobalAddress &addr,
                                                    CoroQueue *waiting_queue,
                                                    CoroContext *cxt,
                                                    int coro_id) {
  auto &node = local_locks[hasher.get_hashed_lock_index(addr)];
...
// read-testing
inline void LocalLockTable::release_local_read_lock(const GlobalAddress &addr,
                                                    bool &res,
                                                    Value &ret_value) {
  auto &node = local_locks[hasher.get_hashed_lock_index(addr)];
