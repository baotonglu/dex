...
RadixCache::RadixCache(int cache_size, DSM *dsm)
...
void RadixCache::add_to_cache(const Key &k, const InternalPage *p_node,
  ...
  for (int i = 0; i < (int)p_node->hdr.partial_len; ++i)
    byte_array.push_back(p_node->hdr.partial[i]);
...
void RadixCache::_insert(const std::vector<uint8_t> &byte_array,
...
next:
  ...
  for (int i = 0; i < (int)hdr->partial.size(); ++i) {
    ...
    if (hdr->depth + i == (int)byte_array.size() - 1 ||
        cur_partial != hdr->partial[i]) {
      // split
      auto partial_len = hdr->depth + i - idx;
      CacheNode *nested_node = nullptr;
      auto new_node =
          new CacheNode(byte_array, idx, partial_len, hdr->partial[i], node,
                        cur_partial, new_entry, nested_node);
      auto &parent_node_entry = parent_node->records[byte_array[idx - 1]];
      auto ret_node = __sync_val_compare_and_swap(&(parent_node_entry.next),
                                                  node, new_node);
      if (ret_node == node) { // cas success
        auto new_hdr = CacheHeader::split_header(hdr, i);
        // update header
        auto ret_hdr = (CacheHeader *)__sync_val_compare_and_swap(
            &(node->header), hdr, new_hdr);
        if (ret_hdr == hdr)
          _safely_delete(ret_hdr); // cas success
        else
          delete new_hdr;
        free_manager->consume_by_node(new_node);
        if (nested_node)
          free_manager->consume_by_node(nested_node);
        ...
        if (ret_node) {
          ...
          node = (CacheNode *)__sync_val_compare_and_swap(
              &(parent_node_entry.next), 0UL, new_node);
          if (node) {
            delete new_node;
            if (nested_node)
              delete nested_node;
      ...
      if (nested_node)
        node_queue->push(nested_node);
      CacheMap::iterator tmp =
          (nested_node ? nested_node->records.find(byte_array.back())
                       : new_node->records.find(cur_partial));
  ...
  if (idx == (int)byte_array.size() - 1) {
    ...
    if (__sync_bool_compare_and_swap(&(node_entry.cache_entry), old_entry,
                                     new_entry)) {
      free_manager->consume_by_node(node);
      free_manager->consume(new_entry->content_size());
      if (old_entry) {
  ...
  // 2.2 internal level
    ...
    if (node_entry.next == nullptr) {
      ...
      auto ret_node =
          __sync_val_compare_and_swap(&(node_entry.next), 0UL, next_node);
      if (ret_node == 0UL) { // cas success
        // record
        node_queue->push(next_node);
        CacheMap::iterator tmp = next_node->records.find(byte_array.back());
        eviction_list.push(
            std::make_pair(&(tmp->second.cache_entry), new_entry));
        free_manager->consume_by_node(node);
...
bool RadixCache::search_from_cache(const Key &k,
...
next:
  ...
  if (r_entry != cache_map.end()) {
    ...
    // ret.push(std::make_pair(std::make_pair(&(r_entry->second.cache_entry),
    // cache_entry), idx + 1));
...
void RadixCache::search_range_from_cache(const Key &from, const Key &to,
  ...
  for (auto k = from; k < to; k = k + 1) {
    ...
    if (e) {
      ...
      result.push_back(RangeCache(leftmost, rightmost, p_ptr, p, depth,
...
void RadixCache::invalidate(volatile CacheEntry **entry_ptr_ptr,
                            CacheEntry *entry_ptr) {
  if (entry_ptr_ptr && entry_ptr &&
      __sync_bool_compare_and_swap(entry_ptr_ptr, entry_ptr, 0UL)) {
...
void RadixCache::_evict() {
  ...
  if (flag) {
    ...
    if (__sync_bool_compare_and_swap(&cache_root, cache_root,
                                     new CacheNode())) {
      std::cout << "Rebuild the Radix Cache" << std::endl;
      free_manager = new FreeMemManager(define::MB * cache_size);
      node_queue = new tbb::concurrent_queue<CacheNode *>();
      node_queue->push(cache_root);
    }
...
//   do {
//     k1 = dsm->getRandomKey();
//   } while(!_search(CacheKey(k1.begin(), k1.begin() + define::keyLen - 1),
//   stk1)); do {
//     k2 = dsm->getRandomKey();
//   } while(!_search(CacheKey(k2.begin(), k2.begin() + define::keyLen - 1),
//   stk2));
...
//     auto cache_entry = (CacheEntry *)r_entry->second.cache_entry;
//     if (cache_entry) {
//       ret.push(SearchRet(&(r_entry->second.cache_entry), cache_entry, idx +
//       1, cache_entry->counter));
//     }
//     node = (CacheNode *)(r_entry->second.next);
//     if (node) {
//       idx ++;
...
void RadixCache::statistics() {
  std::cout << " ----- [IndexCache]: "
            ...
            << " free_size=" << free_manager->remain_size() / define::MB
            << " MB"
            << " node_cnt=" << node_queue->unsafe_size() << " ----- "
            << std::endl;
  std::map<int, int64_t> cnt;
  uint64_t kp_cnt = 0;
  for (auto node_iter = node_queue->unsafe_begin();
       node_iter != node_queue->unsafe_end(); ++node_iter) {
    auto &node = *node_iter;
    auto &cache_map = node->records;
    // auto header = (CacheHeader *)node->header;
    for (auto entry_iter = cache_map.begin(); entry_iter != cache_map.end();
         ++entry_iter) {
      auto cache_entry = (CacheEntry *)entry_iter->second.cache_entry;
      if (cache_entry) {
        int depth = cache_entry->depth;
        if (cnt.find(depth) == cnt.end())
          cnt[depth] = 0;
        cnt[depth]++;
  ...
  std::cout << "cache efficiency="
            << (double)(cache_size * define::MB - free_manager->remain_size()) /
                   kp_cnt
            << " B" << std::endl;
}
