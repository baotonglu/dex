...
/*
  Normal kv entry cache, realize by concurrent_unordered_map. [key prefix ->
  cache entry]
*/
NormalCache::NormalCache(int cache_size, DSM *dsm)
    : cache_size(cache_size), dsm(dsm) {
  free_size = define::MB * cache_size;
  // map_size = 0;
}

void NormalCache::add_to_cache(const Key &k, const InternalPage *p_node,
...
void NormalCache::_insert(const CacheKey &byte_array, CacheEntry *new_entry) {
  ...
  if (__sync_bool_compare_and_swap(&(cache_map[byte_array]), old_entry,
                                   new_entry)) {
    ...
    //       if (free_size.compare_exchange_weak(cur_free_size, cur_free_size -
...
bool NormalCache::search_from_cache(const Key &k,
...
bool NormalCache::_search(CacheKey &byte_prefix, uint8_t last_byte,
                          volatile CacheEntry **&entry_ptr_ptr,
                          CacheEntry *&entry_ptr, int &entry_idx) {
try_upper:
  auto r_entry = cache_map.find(byte_prefix);
  if (r_entry != cache_map.end() &&
      (entry_ptr = (CacheEntry *)r_entry->second)) {
...
void NormalCache::search_range_from_cache(const Key &from, const Key &to,
...
void NormalCache::invalidate(volatile CacheEntry **entry_ptr_ptr,
                             CacheEntry *entry_ptr) {
  if (entry_ptr_ptr && entry_ptr &&
      __sync_bool_compare_and_swap(entry_ptr_ptr, entry_ptr, 0UL)) {
    free_size.fetch_add(sizeof(CacheEntry *) + entry_ptr->content_size() +
                        sizeof(Key));
...
// void NormalCache::_get_a_random_entry(volatile CacheEntry** &entry_ptr_ptr,
// CacheEntry* &entry_ptr) { retry:
//   auto k = dsm->getRandomKey();
//   CacheKey byte_array(k.begin(), k.begin() + define::keyLen - 1);

// try_upper:
//   auto r_entry = cache_map.find(byte_array);
//   if (r_entry != cache_map.end() && (entry_ptr = (CacheEntry
//   *)r_entry->second)) {
//     entry_ptr_ptr = &(r_entry->second);
//     return;
//   }
...
void NormalCache::statistics() {
  ...
  std::cout << "cache efficiency="
            << (double)(cache_size * define::MB - free_size) / kp_cnt << " B"
            << std::endl;
}
