...
void Tree::write_page_and_unlock(char *page_buffer, GlobalAddress page_addr,
  ...
  GlobalAddress version_addr = lock_addr;
  version_addr.offset = version_addr.offset - sizeof(uint64_t);
  dsm->read_dm_sync(reinterpret_cast<char *>(cas_buffer), version_addr,
                    sizeof(uint64_t), cxt);
  uint64_t org_version = *cas_buffer;

  // Write back the page and also increase the version
  RdmaOpRegion rs[2];
  ...
  rs[1].dest = version_addr;
  rs[1].size = sizeof(uint64_t) * 2;
  rs[1].is_on_chip = true;
  auto int_arr = reinterpret_cast<uint64_t *>(rs[1].source);
  int_arr[0] = org_version + 1;
  int_arr[1] = 0;
  dsm->get_rbuf(coro_id).get_cas_buffer();
...
#ifdef LOCK_VERSION
  lock_addr.nodeID = 0;
  lock_addr.offset = lock_index * sizeof(uint64_t) * 2 + sizeof(uint64_t);
#endif
...
int Tree::range_query(const Key &from, const Key &to,
...
re_read:
  ...
  // Get the version-lock of remote page
  dsm->read_dm_sync(reinterpret_cast<char *>(page_buffer), version_addr,
                    sizeof(uint64_t) * 2, cxt);
  auto int_arr = reinterpret_cast<uint64_t *>(page_buffer);
  uint64_t org_version = int_arr[0];
  uint64_t org_lock = int_arr[1];
  ...
  if (result.is_leaf) {
    auto page = (LeafPage *)page_buffer;
#ifdef LOCK_VERSION
    auto version_buffer = dsm->get_rbuf(coro_id).get_cas_buffer();
    dsm->read_dm_sync(reinterpret_cast<char *>(version_buffer), version_addr,
                      sizeof(uint64_t) * 2, cxt);
    auto cur_version = *version_buffer;
    auto cur_lock = version_buffer[1];
    if ((org_version != cur_version) && (cur_lock != 0)) {
      goto re_read;
    }
#else
...
inline bool Tree::acquire_local_lock(GlobalAddress lock_addr, CoroContext *cxt,
                                     int coro_id) {
#ifdef LOCK_VERSION
  auto &node = local_locks[lock_addr.nodeID][(lock_addr.offset - 8) / 16];
#else
